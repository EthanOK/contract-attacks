...
2026 年 6 月 17 日，攻击者对 BSC 上的 **Little Boy Plus（LBP）** 生态发起单笔原子交易攻击。根据 [SlowMist](https://x.com/SlowMist_Team/status/2067424733747122259) 分析，**根因是 `LBPHashrate` 合约在零金额 `transferFrom` 时仍执行 `_update()` → `_harvest()`，向 PancakePair 铸造 LBP 奖励，造成 Pair 余额与储备（reserves）脱节，攻击者再通过 `swap` 抽走 USDT**。攻击者借助 Moolah 闪电贷放大资金规模，最终变现约 **377,642 USDT（~610.555 BNB）**。
...
**漏洞类型**：零金额 `transferFrom` 绕过 allowance 校验 + 转账钩子触发未授权 `_harvest` + Pair **balance ≠ reserves** 套利（与 BY Token 的 `triggerAutoBurn` 路径不同，但同属 AMM 储备失衡类攻击）。
...
LBPHashrate 继承 OpenZeppelin ERC20：`amount == 0` 的 `transferFrom` **不检查 allowance**，但 `_update()` 仍会 `_harvest(pair)` 并调用 **`LBP.mintReward` 铸 LBP**（不是铸 hLBP）。
...
### 4.1 核心问题：零金额 `transferFrom` → 未授权 `_harvest` → Pair 铸币失衡

漏洞位于 **`LBPHashrate._update()`**（合约 `0x5e3cbc82d020be91a989eb747934104e9ab585fe`）：

```solidity
// 攻击者调用 hLBP（LBPHashrate）合约（无需 Pair 授权、无需 allowance）：
...
OpenZeppelin ERC20 `transferFrom` 在 `amount == 0` 时 **跳过 allowance 检查**（零转账视为无资金移动）。但 LBPHashrate 在 `_update()` 中把 **任意转账事件**（含零金额）都挂钩到 `_harvest(from)`，导致：

| 检查项 | 正常预期 | 实际行为 |
|--------|----------|----------|
| `allowance(pair, attacker)` | 应拒绝未授权调用 | `amount=0` → **不检查 allowance** |
| `pair` 是否主动调用 harvest | 仅 Pair / 授权方可触发 | 攻击者借 `transferFrom` **代 Pair 触发** |
...
| allowance | 不涉及 | **OZ 零金额跳过 allowance** |
...
4. **继承 OZ ERC20 未覆盖零金额语义** — 自定义 `_update` 必须意识到 OZ 对 `amount=0` 的 allowance 豁免。
...
| **⑧** | `hLBP.transferFrom(pair, DEAD, 0)` | 零金额 `transferFrom`，**不走 LBP Layer 2.5** | — | **harvest 根因**：`hLBP._harvest` → `LBP.mintReward(pair)` → Pair **token1（LBP）** `balance↑`，`reserves` 不变 |
...
| `hLBP.transferFrom(pair, DEAD, 0)` | hLBP `_update` | **LBP** → Pair（token1） | 池子已操纵；OZ **不查 allowance** |
...
1. OZ `transferFrom`：`amount == 0` → **不校验 allowance**
2. `_update(pair, DEAD, 0)` → `_harvest(pair)`
...
交易中另含 **`skim(Pair)`**（**枢纽步**，见 §5.2 ablation）、Hashrate 多级奖励分配、PolVault 向多地址转出 LBP 等。`skim` 负责把 `lastTransfer` 设为 Pair 以触发 buffer 放大；**SlowMist 根因仍为 Step 3 的零金额 `transferFrom`**，但无 `skim` 则本 tx 无法还 Vault 盈利。
